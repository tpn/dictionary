...
Abstract:
    ...
    bitmaps used by the dictionary component.  This table forms the first level
    hierarchy of word entries, with each bitmap AVL table node capturing the
    second level tree, which will be an AVL table of character histograms.
...
Routine Description:
    ...
    This routine is the bitmap table entry comparison callback invoked by the
...
Arguments:
    ...
    Table - Supplies a pointer to the RTL_AVL_TABLE structure being used for
        the comparison.  This can be converted to the BITMAP_TABLE by either
        CONTAINING_RECORD() or via the Table->TableContext field.

    FirstStruct - Supplies a pointer to the first BITMAP_TABLE_ENTRY structure
        to be used in the comparison.

    SecondStruct - Supplies a pointer to the second BITMAP_TABLE_ENTRY structure
        to be used in the comparison.
...
--*/
{
    ...
    PBITMAP_TABLE_ENTRY_FULL First;
    PBITMAP_TABLE_ENTRY_FULL Second;
    ULONG FirstHash;
    ULONG SecondHash;
    ...
    BitmapTable = CONTAINING_RECORD(Table, BITMAP_TABLE, Avl);

    First = (PBITMAP_TABLE_ENTRY_FULL)RtlOffsetToPointer(FirstStruct, -Offset);
    Second = (PBITMAP_TABLE_ENTRY_FULL)RtlOffsetToPointer(SecondStruct, -Offset);
...
Routine Description:
    ...
    This routine is the bitmap table entry allocation callback invoked by the
...
Routine Description:
    ...
    This routine is the bitmap table entry free callback invoked by the AVL
